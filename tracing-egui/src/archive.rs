...
pub struct LogSpan {
    ...
    pub fields: IndexMap<&'static str, SmartString, FxBuildHasher>,
...
struct HashMapFieldRecordVisitor<'a> {
    target: &'a mut IndexMap<&'static str, SmartString, FxBuildHasher>,
}

impl<'a> HashMapFieldRecordVisitor<'a> {
    fn of(target: &'a mut IndexMap<&'static str, SmartString, FxBuildHasher>) -> Self {
        Self { target }
    }
}
...
impl LogSpan {
    ...
    pub fn show_fields(&self, ui: &mut egui::Ui) {
        for (field, value) in &self.fields {
            // ui.add(egui::Label::new(format!("{}: {}", field, value)).monospace());
